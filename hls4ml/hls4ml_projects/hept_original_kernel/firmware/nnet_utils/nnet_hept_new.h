#ifndef NNET_HEPT_NEW_H_
#define NNET_HEPT_NEW_H_
...
struct hept_config {
    ...
    static const unsigned eps_power = 10;
...
void permute_1023(data_T input[CONFIG_T::n_heads * CONFIG_T::batch_size * CONFIG_T::seq_len * CONFIG_T::dim_per_head],
...
void init_invert_table(typename CONFIG_T::inv_table_t table_out[N_TABLE]) {
    float epsilon = (float)1.0 >> CONFIG_T::eps_power;
...
template <class data_T, class res_T, typename CONFIG_T>
void mask_and_normalize(data_T kernel[CONFIG_T::batch_size * CONFIG_T::n_heads * CONFIG_T::seq_len * CONFIG_T::seq_len],
    ...
    typename CONFIG_T::table_t denom = 0;
...
void bmm(
    ...
    for (unsigned a = 0; a < A; a++) {
        ...
        for (unsigned b = 0; b < B; b++) {
            #pragma HLS UNROLL
            nnet::dense<res_T, res_T, typename CONFIG_T::dense_conf_qkv>(&qk[(a * B * B + b * B)], out_buffer,
                                                                &value[(a * B * C)], biases);
...
template <class data_T, class res_T, typename CONFIG_T>
void hept_new(data_T query[CONFIG_T::n_heads * CONFIG_T::batch_size * CONFIG_T::seq_len * CONFIG_T::dim_per_head],
          data_T key[CONFIG_T::n_heads * CONFIG_T::batch_size * CONFIG_T::seq_len * CONFIG_T::dim_per_head],
    ...
    data_T q_perm[CONFIG_T::batch_size * CONFIG_T::n_heads * CONFIG_T::seq_len * CONFIG_T::dim_per_head];
    data_T k_perm[CONFIG_T::batch_size * CONFIG_T::n_heads * CONFIG_T::seq_len * CONFIG_T::dim_per_head];
    data_T v_perm[CONFIG_T::batch_size * CONFIG_T::n_heads * CONFIG_T::seq_len * CONFIG_T::dim_per_head];
    res_T qk[CONFIG_T::batch_size * CONFIG_T::n_heads * CONFIG_T::seq_len * CONFIG_T::seq_len];
    res_T qk_mask[CONFIG_T::batch_size * CONFIG_T::n_heads * CONFIG_T::seq_len * CONFIG_T::seq_len];

    nnet::permute_1023<data_T, data_T, CONFIG_T>(query, q_perm);
    nnet::permute_1023<data_T, data_T, CONFIG_T>(key, k_perm);
    nnet::permute_1023<data_T, data_T, CONFIG_T>(value, v_perm);
    nnet::pairwise_dist_sq_rbf<data_T, res_T, CONFIG_T>(q_perm, k_perm, qk);
    nnet::mask_and_normalize<data_T, res_T, CONFIG_T>(qk, padding_mask, qk_mask);
    nnet::bmm<data_T, res_T, CONFIG_T>(qk_mask, v_perm, output);
}
